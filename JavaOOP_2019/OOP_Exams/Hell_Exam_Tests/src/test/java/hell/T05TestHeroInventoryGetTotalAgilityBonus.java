...
public class T05TestHeroInventoryGetTotalAgilityBonus {
    ...
    private static final String WRONG_RESULT = "Wrong result from method getTotalAgilityBonus";

    @Test
    public void test() throws NoSuchMethodException, IllegalAccessException, InvocationTargetException, InstantiationException {
        Item mockedItem = Mockito.mock(Item.class);
        Mockito.when(mockedItem.getAgilityBonus())
        ...
        Mockito.when(mockedItem2.getAgilityBonus())
                .thenReturn(MOCKED_ITEM_VALUE);
        Mockito.when(mockedItem.getName())
        ...
        Assert.assertEquals(WRONG_RESULT,
                MOCKED_ITEM_VALUE * 2, inventory.getTotalAgilityBonus());
