...
SELECT CONCAT('Critica Problem With ', f.`name`,'!'),
...
FROM
    files AS f
WHERE
    f.`id` NOT IN (SELECT DISTINCT
            (f1.`parent_id`)
        FROM
            files AS f1
        WHERE
            f1.`parent_id` IS NOT NULL)
ORDER BY f.`id`;

SELECT 
    r.`id`, r.`name`, nt.`count`
FROM
    repositories AS r
        JOIN
    (SELECT 
        i.repository_id, COUNT(i.id) AS `count`
    FROM
        issues AS i
    GROUP BY i.`repository_id`) AS nt ON nt.`repository_id` = r.`id`
ORDER BY nt.`count` DESC , r.`id`
LIMIT 5;

SELECT 
    r.`id`, r.`name`, nt.`count`, nt2.`count`
FROM
    repositories AS r
        JOIN
    (SELECT 
        c.`repository_id`, COUNT(c.`id`) AS `count`
    FROM
        commits AS c
    GROUP BY c.`repository_id`) AS nt ON r.`id` = nt.`repository_id`
        JOIN
    (SELECT 
        rc.`repository_id`, COUNT(rc.`contributor_id`) AS `count`
    FROM
        repositories_contributors AS rc
    GROUP BY rc.`repository_id`) AS nt2 ON r.`id` = nt2.`repository_id`
ORDER BY nt2.`count` DESC , r.`id`
LIMIT 1;
...
FROM
	...
	LEFT JOIN
    (SELECT 
        c.`contributor_id`, COUNT(c.`id`) AS `count`
    FROM
        commits AS c
    JOIN issues AS i ON c.`issue_id` = i.`id`
    WHERE
        c.`contributor_id` = i.`assignee_id`
    GROUP BY c.`contributor_id`
) AS nt ON u.`id` = nt.`contributor_id`
ORDER BY nt.`count` DESC, u.`id`;

SELECT 
    SUBSTRING(f.`name`,
        0,
        POSITION('.' IN f.`name`)) AS `name`,
    f.`parent_id`,
    f1.`name`
FROM
    files AS f
        JOIN
    files AS f1 ON f.`parent_id` = f1.`id`
WHERE
    f.parent_id = f1.`id`
        AND f.`id` = f1.`parent_id`;
...
FROM
    repositories AS r
        JOIN
    (SELECT 
        c.`repository_id`, COUNT(c.`contributor_id`) AS `count`
    FROM
        commits AS c
    GROUP BY c.`repository_id`) AS nt ON r.`id` = nt.`repository_id`
ORDER BY nt.`count` DESC , r.`id`;
...
DELIMITER $$
CREATE PROCEDURE udp_commit(username VARCHAR(50), password VARCHAR(50), message VARCHAR(255), issue_id INT)
BEGIN
	DECLARE user_id INT;
    DECLARE repository_id INT;
    
	SET user_id := (SELECT u.`id` FROM users AS u WHERE u.`username` = username);
    SET repository_id := (SELECT i.`repository_id`FROM issues AS i WHERE i.`id` = issue_id);
...
BEGIN
    ...
    FROM files AS f
    WHERE f.`title` LIKE (CONCAT('%',extention))
    ORDER BY f.`id`;
END;
DELIMITER $$
...
DELIMITER $$
CREATE PROCEDURE udp_findyextension(extension VARCHAR(100))
BEGIN
	SELECT f.`id`, f.`name`, CONCAT(f.`size`, 'KB')
    FROM files f
